...
class TronBattle {
    ...
    private static int count_self = 0;
    private static int count_other = 0;
        ...
        while (true) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if(i == P){
                    ...
                    //remove edges to that node so won't go back again
                    /**
                    List<Node> neibs = graph.neighbors(current);
                    System.err.println("Debug remove self" + neibs.size());
                    Iterator<Node> iterator = neibs.iterator();
                    while(iterator.hasNext()){
                        //remove edges of both direction from past node to its neiboughers
                    	Node n = iterator.next();
                    	Tile t = (Tile)n.getData();
                    	Edge ed = new Edge(n,current,1);
                    	System.err.println("Debug remove " + t.getX() + " " + t.getY() + " to " + X1 + " " + Y1 );
                    	graph.removeEdge(ed);
    ...
    public static Edge floodfill(int x, int y, AdjacencyList graph){
    	...
    	List neibs = graph.neighbors(current);
    	System.err.println("Debug neib " + neibs.size() + " " + x + " " + y );
        Iterator<Node> iterator = neibs.iterator();
        while(iterator.hasNext()){
    ...
    //the leaf node value is how far it's from the source, the further the higher
    //leaf nodes are those don't have any child
    public static int alphaBeta(AdjacencyList graph, Node source, Integer depth, Integer alpha, Integer beta, Boolean max, int count){
    	...
    	if(max == true){
        	...
        	for(Node<MiniMaxState> n : graph.neighbors(current)){
        		value = Math.max(value, alphaBeta(graph, n, depth, alpha, beta, false, count+1));
        		//update graph
        	...
        	for(Node<MiniMaxState> n : graph.neighbors(current)){
        		value = Math.min(value, alphaBeta(graph,n,depth, alpha,beta,true,count+1));
        		//update graph
        		current.getData().setValue(value);
        		if(value <= alpha){
        			return value;
